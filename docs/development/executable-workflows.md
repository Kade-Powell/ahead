...
| AI authority | `docs/guide/acceptable-ai-use.md` |
| Human-readable flows | `docs/guide/workflows/*.md` |
...
The Pi package copies the canonical Constitution, `docs/guide/**/*.md`, and `docs/evidence/**/*.md` into a generated runtime reference catalog. Each entry declares its audience, authority, and distribution. Maintainer material under `docs/development` remains repository-only. Runtime references are not injected into every prompt: the adapter recommends documents applicable to the active phase, lets humans read them through `/ahead-guide`, and lets AI retrieve a specific source through `ahead_get_reference`. This keeps the binding prompt small while making the practitioner framework, evidence, and original page-level provenance available on demand.
...
Review presentation follows the same rule. The core requires snapshot-bound findings, dispositions, and review gates; a host maps portable paths and locations to a terminal viewer, VS Code diff/comment UI, or GitHub review API. See `docs/development/review-workbench.md`.
